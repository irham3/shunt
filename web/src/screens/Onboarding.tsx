...
/** Problem Ã¢â€ â€™ outcome, paired Ã¢â‚¬â€ one compact scan, real product language. */
...
const LANES = [
  { ...DEFAULT_BUCKETS[0], desc: "In your wallet for spending. Anchor cash-out is outside this testnet scope." },
  ...
  { ...DEFAULT_BUCKETS[3], desc: "An optional lane in the current UI; production scope starts with USDC routing." },
];

const STEPS = [
  { title: "Connect", body: "Freighter, Albedo, or xBull. Connect a wallet to test the flow." },
  { title: "Set rules", body: "Needs / Savings / Buffer / Invest, saved on-chain." },
  { title: "Income lands", body: "Payment link, Top Up, or transfer Ã¢â‚¬â€ detected in seconds." },
  { title: "One tap", body: "Review the breakdown, sign. Nothing moves without you." },
  { title: "Preview the route", body: "Review the allocation before signing. Production routing remains the next build stage." },
];
...
// cost Ã¢â‚¬â€ an anchor charges its own fee on top, so we don't claim a multiple
// "cheaper than remittance" here.
const STATS = [
  { value: 32, decimals: 0, suffix: "", label: "Soroban contract tests" },
  { value: 4, decimals: 0, suffix: "", label: "Core allocation lanes" },
  { value: 10, decimals: 0, suffix: "%", label: "Early-exit penalty Ã¢â€ â€™ Buffer" },
];

const FEES = [
  { label: "Cash-out to fiat", rate: "Not live" },
  { label: "Top Up to USDC", rate: "Testnet" },
  { label: "Invest / Convert", rate: "Out of scope" },
  { label: "Savings withdrawal", rate: "Testnet" },
];
  ...
  return (
    <div className="lp-stat">
      <div className="lp-stat-value">
        {/* Render the real figure at rest Ã¢â‚¬â€ never count up from 0, so a
...
export function Onboarding() {
  ...
  return (
    <AnimatedBackground aurora threads>
      ...
      >
        ...
        {/* <Reveal variant="up" delay={0.24}>
          <div className="muted" style={{ fontSize: 13, letterSpacing: "0.01em", marginTop: 2 }}>
            USDC Ã‚Â· Non-custodial Ã‚Â· Built on Stellar
          </div>
        </Reveal> */}
      ...
      <div style={{ overflow: "hidden", padding: "18px 0", borderTop: "1px solid var(--color-border-subtle)", borderBottom: "1px solid var(--color-border-subtle)" }}>
        <div className="lp-marquee-track">
          {[...Array(2)].flatMap((_, dup) =>
            ["Stellar Testnet", "Soroban Ã‚Â· Rust", "React + TypeScript", "PWA Ã¢â‚¬â€ Mobile First", "32 Contract Tests Passing", "USDC-only first scope"].map((t, i) => (
      ...
      {/* 3. The loop: four lanes Ã¢â‚¬â€ de-templated, no icon-in-a-box */}
      <section id="loop" className="lp-section" style={{ padding: "72px 24px", display: "flex", flexDirection: "column", gap: 28 }}>
        <Reveal variant="up" style={{ textAlign: "center" }}>
          ...
          <p className="muted" style={{ fontSize: 16, marginTop: 8 }}>Money in, structured on-chain, with production cash-out kept out of this testnet scope.</p>
        </Reveal>
      ...
      {/* 6. Current scope, no yield promise */}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 16, alignItems: "baseline" }}>
            <div>
              ...
              <p className="muted" style={{ margin: "6px 0 0", fontSize: 13, maxWidth: 360 }}>
                This is a testnet preview. Anchor access and production fee schedules are not live yet.
              </p>
            </div>
      ...
      <section className="lp-section" style={{ textAlign: "center", padding: "48px 24px 96px", display: "flex", flexDirection: "column", alignItems: "center", gap: 22 }}>
        <Reveal variant="scale">
          <h2 style={{ fontSize: "var(--text-h1)", margin: 0 }}>Preview the split before payday.</h2>
        </Reveal>
        <Reveal variant="scale" delay={0.12}>
          <button
            ...
            onClick={() => nav("/connect")}
          >
            Try testnet <i className="ph ph-arrow-right" />
          </button>
        </Reveal>
      </section>
      ...
      <footer className="lp-section" style={{ padding: "28px 24px 44px", borderTop: "1px solid var(--color-border-subtle)", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 16, alignItems: "center" }}>
        <div className="lp-brand muted" style={{ fontSize: 13, fontWeight: 500 }}>
          <Logo size={22} />
          Shunt Ã‚Â· income in, structured by code, income out
        </div>
